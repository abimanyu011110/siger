  ...
  <div class="col-sm-12">
    <div class="widget-box">
      ...
      <div class="widget-header">
        <h4 class="widget-title">Edit Data Misi</h4>
      </div>

      <div class="widget-body">
        <div class="widget-main">
          <form class="form-horizontal" action="{{route('misi.update', $misi->id)}}" method="post">
          <input type="hidden" name="_method" value="put">
          <input type="hidden" name="misi_id" value="{{$misi->id}}">
            {!! csrf_field() !!}
          ...
          <div class="col-sm-10">
            <select name="visi_id" class="form-control" id="visi_id">
              ...
              @foreach($visi as $key => $value)
                <option value="{{$key}}" {{$misi->visi_id == $key ? 'selected' : ''}}>{{$value}}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="form-group  @if($errors->has('nama_misi')) has-error @endif"">
          <label for="nama_misi" class="col-sm-2 control-label">Nama Misi</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" name="nama_misi" value="{{$misi->nama_misi}}">
