  ...
  <div class="col-sm-12">
    <div class="widget-box">
      ...
      <div class="widget-header">
        <h4 class="widget-title">Tambah Data OPD</h4>
      </div>

      <div class="widget-body">
        <div class="widget-main">
          <form class="form-horizontal" action="{{route('opd.store')}}" method="post">
            {!! csrf_field() !!}
          <div class="form-group">
            <label for="urusan_id" class="col-sm-2 control-label">Pilih Urusan</label>
              ...
              @foreach($urusan as $key => $value)
                <option value="{{$key}}" {{old('urusan_id') == $key ? 'selected' : ''}}>{{$value}}</option>
              @endforeach
                </select>
              </div>
          </div>
        ...
        <div class="form-group  @if($errors->has('kepala_opd')) has-error @endif"">
          <label for="kepala_opd" class="col-sm-2 control-label">Kepala OPD</label>
            <div class="col-sm-6">
              <input type="text" class="form-control" name="kepala_opd" value="{{old('kepala_opd')}}">
