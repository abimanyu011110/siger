  ...
  <div class="col-sm-12">
    <div class="widget-box">
      ...
      <div class="widget-header">
        <h4 class="widget-title">Tambah Data Risiko Pemerintah Daerah</h4>
      </div>

      <div class="widget-body">
        <div class="widget-main">
          <form class="form-horizontal" action="{{route('baganrisiko.store')}}" method="post">
            {!! csrf_field() !!}
          ...
          <div class="col-sm-6">
            <select name="kategori_id" class="form-control" id="kategori_id">
              ...
              @foreach($kategori as $key => $value)
                <option value="{{$key}}" {{old('kategori_id') == $key ? 'selected' : ''}}>{{$value}}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="form-group  @if($errors->has('nama_risiko')) has-error @endif"">
          <label for="nama_risiko" class="col-sm-2 control-label">Nama Risiko</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" name="nama_risiko" value="{{old('nama_risiko')}}">
