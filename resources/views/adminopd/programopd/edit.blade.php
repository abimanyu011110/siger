  ...
  <div class="col-sm-12">
    <div class="widget-box">
      ...
      <div class="widget-header">
        <h4 class="widget-title">Edit Data Program {{Auth::user()->nama}}</h4>
      </div>

      <div class="widget-body">
        <div class="widget-main">
          <form class="form-horizontal" action="{{route('programopd.update', $program->id)}}" method="post">
          ...
          <div class="col-sm-10">
            <select name="opd_id" class="form-control" id="opd_id">
              @foreach($opd as $key => $value)
                <option value="{{$key}}" {{$program->opd_id == $key ? 'selected' : ''}}>{{$value}}</option>
              @endforeach
            </select>
          </div>
        </div>
